# source('./Code/Figures/F2-Kernel.R')
# ~~~~~~~~~~~~~~~~~~~ LIBRARIES ~~~~~~~~~~~~~~~~~~~ #
# Libraries
library(raster)
library(sf)
...
# EGSL FIGURE
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= #
jpeg('./figures/kernel.jpg', width = 1920, height = 1280, res = 300, pointsize = 6)
...
plotEGSL(layers = 'egslSimple', borders = '#C7CBCE66', prj = world, extent = 'egslSimple')
...
f1 <- MASS::kde2d(x = drivers$Hypoxia[notEmpty],
...
polygon(x = c(dens$x, dens$x[length(dens$x)], 0), y = c(dens$y,0,0), col = colsPalette[5], border = colsPalette[5], lwd=1)
...
polygon(x = c(dens$y, 0, 0), y = c(dens$x,dens$x[length(dens$x)],0), col =colsPalette[5], border = colsPalette[5], lwd=1)
