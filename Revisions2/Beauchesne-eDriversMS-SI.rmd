...
<!--
rmarkdown::render('./Revisions/Beauchesne-eDriversMS-SI.rmd')
-->

# Drivers description


Table S1. List of drivers currently available on *eDrivers* along with their respective acronym used in the figures \textcolor{red}{\sout{in}} \textcolor{blue}{of} the supplementary material.
...
Oceans are the largest reservoir\textcolor{blue}{s} and sink\textcolor{blue}{s} of atmospheric carbon dioxide ($CO_2$). Its uptake increases seawater acidity and \textcolor{red}{\sout{the}} lowers \textcolor{blue}{the} saturation state \textcolor{blue}{of waters} with respect to calcite ($\Omega_C$) and aragonite ($\Omega_A$), the two most common $CaCO_3$ polymorphs \textcolor{blue}{that constitute} \textcolor{red}{\sout{constituting}} the shells and skeleton\textcolor{blue}{s} of many marine organisms [@mucci2017]. \textcolor{blue}{Ocean acidification} \textcolor{red}{\sout{This}} can have deleterious effects on carbonate-secreting organisms (*e.g.* mollusks and crustaceans) and certain physiological processes in non-calcifying organisms [@fabry2008; @kroeker2013].

When $\Omega_C$ or $\Omega_A$ decrease below 1, water becomes undersaturated and corrosive to the skeletal minerals of carbonate-secreting organisms. Note that if $\Omega_A$ $< 1$, the waters may still be supersaturated with respect to calcite since it is 50% more soluble than aragonite. However, organisms have wide ranging responses to \textcolor{blue}{changes in $\Omega_C$ and $\Omega_A$} \textcolor{red}{\sout{saturation state}}. For example, most corals stop calcifying at $\Omega_A$ $< 2$ [@kleypas2006; @bove2019], while other organisms may have adapted to precipitate $CaCO_3$ even when seawater is undersaturated with respect to calcite [$\Omega_C$ $< 1$ or $\Omega_A$ $< 0.65$; e.g. @uthicke2016].

We used $\Omega_{A}$ \textcolor{red}{\sout{saturation state}} to characterize ocean acidification in the bottom waters of the St. Lawrence. The data come from the Department of Fisheries and Oceans' (DFO) Atlantic Zone Monitoring Program [AZMP; @galbraith2018] surveys and DFO's multispecies surveys [@bourdages2018] collected in August-September of 2017 [@starr2019]. \textcolor{blue}{$\Omega_{A}$, pH, and dissolved oxygen ($O_2$) were measured at 117 stations in the Estuary and Gulf of St. Lawrence in the summer of 2018. The majority of sampling stations were located in the Southern Gulf ($n = 84$), the Estuary and Northern Gulf ($n = 33$).}

The carbonate \textcolor{red}{\sout{chemistry}} \textcolor{blue}{speciation} was determined through pH and \textcolor{blue}{total alkalinity} (TA) measurements. Samples for pH and \textcolor{red}{\sout{total alkalinity}} TA were collected \textcolor{red}{\sout{under a bubble free and}} \textcolor{blue}{with} no head space \textcolor{red}{\sout{conditions}} in\textcolor{red}{\sout{to}} 500 $mL$ borosilicate glass flasks\textcolor{red}{\sout{.}}\textcolor{blue}{, and} 250 $\mu L$ of \textcolor{red}{\sout{the}} saturated $HgCl_2$ solution was
added to \textcolor{blue}{each} sample\textcolor{red}{\sout{,}} and processed following the "Guide to best practices for Ocean
...
measurement\textcolor{red}{\sout{s}} using a TRIS (Tris (hydroxymethyl) -aminomethane) buffer prepared
...
\textcolor{red}{\sout{dilute}} \textcolor{blue}{0.1M} HCl titrant
\textcolor{red}{\sout{0.1M}} in \textcolor{red}{\sout{a solution of}}
\textcolor{blue}{0.6M} NaCl \textcolor{red}{\sout{of 0.6M}} was calibrated using CRM provided by Professor Andrew G. Dickson.
...
[@lewis1998] with measured pH, \textcolor{red}{\sout{total alkalinity}} \textcolor{blue}{TA}, soluble reactive \textcolor{blue}{phosphate}
and silicate concentrations as input parameters.
\textcolor{red}{\sout{$\Omega_{A}$, pH, dissolved oxygen ($O_2$) and pressure were sampled at 117
...
To account for \textcolor{blue}{the} low sample size in the Northern Gulf, we used the correlation
...
respiration of organic matter [@mucci2011]. In deep waters\textcolor{blue}{, found in the Laurentian, Anticosti and Esquiman Channels}, variations of
$\Omega_{A}$ are largely governed by the addition of metabolic $CO_2$, whereas\textcolor{blue}{,} near the air-sea interface\textcolor{blue}{,} $O_2$ is replenished faster than $CO_2$ can escape
...
strong when deep Channel\textcolor{red}{\sout{s}} \textcolor{blue}{waters} (*Deep* model: *p-value*: < 0.01; $O_2$: 0.20; $R^2$:
...
increasing $O_2$ in \textcolor{blue}{the} deep Channels. This stronger correlation is likely caused by the lack of gas exchange with the atmosphere or the surface mixed layer and the importance of metabolic $CO_2$ in deep waters, some of which may have mean ages of up to 20 years [@mucci2011]. In the Northern Gulf, the acidification \textcolor{red}{\sout{is following}} \textcolor{blue}{trends with} depth and distance from the Cabot Strait along the Laurentian, Anticosti and Esquiman Channels due to the progressive oxygen depletion and metabolic carbon dioxide accumulation [@mucci2011].

There are likely different dynamics at play in the Southern Gulf, such as waters with lowered alkalinity and salinity caused by the \textcolor{blue}{surface, seaward-flowing Gaspé Current} \textcolor{red}{\sout{St. Lawrence plume}}. \textcolor{red}{\sout{Such}} \textcolor{blue}{These} water\textcolor{blue}{s} \textcolor{red}{\sout{masses}} \textcolor{red}{\sout{are}} have lower buffering capacities to decreasing pH and $\Omega_{A}$. \textcolor{red}{\sout{Also}} \textcolor{blue}{Furthermore}, the accumulation of metabolic carbon dioxide produced by rapid oxic degradation of organic matter in the sandy and permeable sediments of the Magdalen Shelf or \textcolor{red}{\sout{through}} \textcolor{blue}{in} the \textcolor{red}{\sout{St. Lawrence plume}} \textcolor{blue}{Gaspé current} could decrease $\Omega_{A}$ in the Southern Gulf (Starr and Chassé, *personal communication*).

We thus divided the \textcolor{red}{\sout{analyses}} \textcolor{blue}{and their analysis} between the Southern and Northern St. Lawrence. The St. Lawrence was divided \textcolor{red}{\sout{using}} \textcolor{blue}{along} the 350m isobath on the southern slope of the Laurentian Channel, from the Cabot Strait to the tip of the Gaspé Peninsula. \textcolor{blue}{The} $\Omega_A$ distribution in the Southern St. Lawrence was modeled
...
Lawrence, there is a robust time-series on oxygen concentration\textcolor{blue}{s} in the water column \textcolor{red}{\sout{from}} \textcolor{blue}{available through}
...
and have a 100 $km^2$ resolution. For more details, refer to the hypoxia \textcolor{blue}{data layer}
...
Based on the wide range of $\Omega_A$ levels at which organism\textcolor{blue}{s} are negatively
affected (see above), we built an index of acidification stress ($A_s$)
ranging between 0 and 1 that slowly \textcolor{red}{\sout{begins increasing}} \textcolor{blue}{increases} as $\Omega_A$ \textcolor{blue}{decreases to} $\approx$
...
The data used to characterize hypoxia
\textcolor{red}{\sout{uses oxygen saturation in \% and}}
come from
the Department of Fisheries and Oceans’ (DFO) Atlantic Zone Monitoring Program
[AZMP; @galbraith2018] survey\textcolor{red}{\sout{s}} in late spring and fall
\textcolor{blue}{of 2013 to 2017}, as well as from \textcolor{red}{\sout{annual}}
DFO’s \textcolor{blue}{annual} multispecies surveys for the northern Gulf in August and for the Magdalen
...
Oxygen concentration \textcolor{red}{\sout{is}} \textcolor{blue}{were} measured at every station using an oxygen probe
(Sea-Bird SBE43) mounted on \textcolor{red}{\sout{the}} \textcolor{blue}{a} CTD. The probe is calibrated against seawater
samples \textcolor{red}{\sout{that are}} \textcolor{blue}{taken directly from the Niskin bottles at every cast and} analyzed by Winkler titration \textcolor{red}{\sout{on every cast}} [see @blais2019 for calibration procedure]. The data used is the last depth
sampled on the CTD profile, which \textcolor{blue}{is} typically \textcolor{red}{\sout{corresponds to}} ~10m above the
bottom. \textcolor{blue}{Note that this depth does not necessarily correspond to the depth of the oxygen minimum. The oxygen minimum is typically found along the 27.25 isopycnal, often situated as much as 100 m above bottom in the Lower Estuary.} Oxygen concentration ($\mu M$ $L^{-1}$) is converted to oxygen
saturation\textcolor{blue}{,} taking into account the salinity and temperature at the selected
depth.
...
100 $km^2$ resolution. According to @diaz1995, severe hypoxia \textcolor{red}{\sout{can be observed}} \textcolor{blue}{occurs}
when dissolved oxygen falls below 2 $ml$ $L^{-1}$, \textcolor{red}{\sout{which corresponds}} to 62.5 $\mu mol$ $L^{-1}$ \textcolor{red}{\sout{and}} \textcolor{blue}{or} ~20% saturation. This is considered as the level
necessary to maintain most animal life. \textcolor{red}{\sout{Similarly}} \textcolor{blue}{Accordingly}, @chabot2008 studied the
effects of hypoxia on the energy budget of cod in the St. Lawrence and found
that behavioural effects began manifesting \textcolor{blue}{themselves} below 70% oxygen saturation and that
...
increases below 70% oxygen saturation\textcolor{red}{\sout{,}} \textcolor{blue}{and} increases acutely as it \textcolor{red}{\sout{approaches the 20\% oxygen saturation threshold and}} reaches its peak intensity between 20 and 30% oxygen saturation:
...
### Bottom\textcolor{blue}{-water} temperature anomalies

The data used to characterize bottom\textcolor{blue}{-water} temperature anomalies come from the
...
characterize the bottom\textcolor{blue}{-water} temperature climatology and anomalies in this document.
For more details, refer to @galbraith2018.

Bottom\textcolor{blue}{-water} temperatures are interpolated in the Gulf using
...
interpolated at each 1 m depth layer on a 2 km resolution grid. Bottom\textcolor{blue}{-water}
...
conditions \textcolor{blue}{between 1981 and 2010}, to measure an annual index of stress associated with extreme
...
comparison\textcolor{blue}{s} between areas of the St. Lawrence with different temperature ranges.
...
95th percentile\textcolor{red}{\sout{s}} \textcolor{blue}{values}. Anomalies were divided into positive and negative anomalies
and the absolute value of anomalies were used as an indicator of the intensity
of bottom\textcolor{blue}{-water} temperature anomalies. The mean anomaly intensity between 2013 and
2017 for each grid cell was used to generate the final index of bottom\textcolor{blue}{-water}
temperature anomalies.



### Surface\textcolor{blue}{-water} temperature anomalies

The data used to characterize surface\textcolor{blue}{-water} temperature anomalies come from the
Department of Fisheries and Oceans’ (DFO) Atlantic Zone Monitoring Program
[AZMP; @galbraith2018]. We provide a brief summary of data and methods to
characterize the surface\textcolor{blue}{-water} temperature climatology and anomalies in this document.
For more details, refer to @galbraith2018.

The surface\textcolor{blue}{-water} layer is characterized using sea surface\textcolor{blue}{-water} temperature (SST) monthly
...
Surface\textcolor{blue}{-water} temperature anomalies were characterized following the same method used
for bottom\textcolor{blue}{-water} temperature anomalies. Only the months of May to November were
...
annual surface\textcolor{blue}{-water} temperature anomaly intensity in each grid cell. The mean
anomaly intensity between 2013 and 2017 for each grid cell was used to generate
the final index of surface\textcolor{blue}{-water} temperature anomalies.


### Sea\textcolor{blue}{-}level rise

The data used to characterize sea\textcolor{blue}{-}level rise risk come from the global
...
Sea\textcolor{blue}{-}level rise was characterized by @nicholls2010 using NASA's satellite
...
The rate of sea\textcolor{blue}{-}level rise ($mm/year$) was measured between 1992 and 2012 and
...
only positive sea\textcolor{blue}{-}level rise is likely to cause environmental stress.
...
\textcolor{red}{\sout{Aquaculture data comes from a variety of sources in the St. Lawrence because
aquaculture sites are mostly managed at the provincial level.
We therefore had to gather the data on aquaculture sites from the 5 provinces dividing the
St. Lawrence}}
\textcolor{blue}{As aquaculture sites are managed at the provincial level in the St. Lawrence, data on aquaculture sites come from various sources}
...
Considering this, we only \textcolor{red}{\sout{considered}} \textcolor{blue}{included} invertebrates aquaculture for the
aquaculture driver layer. However, if fish or algae farming were to become more
important, these driver\textcolor{blue}{s} should be incorporated in future analyses as individual
...
the \textcolor{red}{\sout{actual}} location of sites to characterize the distribution of this driver.
...
we considered aquaculture as binary presence-absence data \textcolor{red}{\sout{in our analyses}}.


### Coastal development


\textcolor{red}{\sout{We used lights at night as a proxy of coastal infrastructure development, as
terrestrial stable lights at night represent light from human settlements and
industrial sites with electricity.}}
\textcolor{blue}{Terrestrial stable lights at night mostly represent light from human settlements and industrial sites with electricity.
We thus used lights at night as a proxy of coastal infrastructure development.}
...
this layer. The data obtained was aggregated at the watershed scale. \textcolor{red}{\sout{and spread}}
\textcolor{blue}{Spread} into coastal and marine environments was modeled using a diffusive plume model
...
overview of the spatial distribution and intensity of fishing activities \textcolor{red}{\sout{in the
St. Lawrence}}. The data we used span 6 years from 2010 to 2015 and detail
218323 fishing events (36387 $\pm$ 3147 fishing events per year). There were 31
targeted species and a total of 53 \textcolor{red}{\sout{caught}} species \textcolor{blue}{caught} in the dataset.
...
This categorization \textcolor{red}{\sout{therefore}} divides the fisheries data into 5 distinct
driver layers characterizing fishing activities.

Gear types can \textcolor{red}{\sout{also}} be further classified into fixed or mobile engines based
...
uncertainty associated with locations, \textcolor{red}{\sout{and}} the fact that mobile engines can be
tracted over several kilometers during fishing activities, and \textcolor{red}{\sout{that}} \textcolor{blue}{because} we do not
...
\textcolor{red}{\sout{In order to}} \textcolor{blue}{To} characterize the intensity of fishing activities ($FI$), we used
...
\textcolor{red}{\sout{was}} \textcolor{blue}{were} from November 2010 to December 2011.
...
- Nutrient pollution
- Organic pollution
- \textcolor{red}{\sout{Sea}} Bottom\textcolor{blue}{-water} temperature anomalies
- Shipping
