---
title: Next Generation Planning -- Characterizing Exposure to and Shared Knowledge on Drivers of Ecological Change in the St. Lawrence System
...
<!--
rmarkdown::render('./Revisions2/Beauchesne-eDriversMS.Rmd', 'pdf_document')
-->

<!--
To discuss with co-authors

- New article title
- Should we present how the platform is used in the analyses or really keep
  that portion fully separate? If we include it, modify answer to question 2
  comment 1
...
wealth of services sustaining numerous economic sectors. ~~\textcolor{red}{These ecosystems are}~~
\textcolor{blue}{This ecosystem is}
...
systematically characterize the distribution and intensity of drivers \textcolor{blue}{of change} in the
...
knowledge on drivers \textcolor{blue}{of change} in support of science and management.~~ We gathered data on
...
cumulative exposure regime that show that certain drivers \textcolor{blue}{of change} typically co-occur
...
regimes at the head of the Laurentian Channel.
\textcolor{blue}{Sharing the knowledge acquired emerged as a priority to minimize future
environmental assessment efforts. We thus launch *eDrivers*, an open knowledge
platform gathering experts committed to structuring, standardizing and sharing
knowledge on drivers of change in support of science and management.}
*eDrivers* was built on a
...
The St. Lawrence System, formed by one of the largest estuaries in the world\textcolor{red}{\sout{, the St. Lawrence Estuary,}}
and a vast interior sea\textcolor{red}{\sout{, the Gulf of St. Lawrence}}, is
...
Lawrence System provides a wealth of ecosystem services that have \textcolor{red}{\sout{historically and contemporarily}}
...
As elsewhere in the world [see @halpern2015a; @halpern2019], this intensifying human footprint
will likely result in increasingly intricate environmental exposure regimes,
*i.e.* suites of overlapping \textcolor{red}{\sout{and potentially interacting}} environmental drivers \textcolor{blue}{of change}
threatening ecosystems, habitats or ecological communities. Drivers \textcolor{blue}{of change}, often referred to as stressors or pressures \textcolor{blue}{and hereafter called drivers for simplicity}, are any externalities that affect
environmental processes and disturb natural systems. Drivers may originate from
natural or human-induced biophysical processes (*e.g.* sea surface\textcolor{blue}{-water} temperature
...
\textcolor{blue}{co-occurring} drivers is the largest uncertainty when studying or predicting environmental impacts [@darling2008; @cote2016]. Multiple drivers can combine non-linearly and result in effects that are greater (synergistic effect) or lower (antagonistic effect) than the sum of individual effects
[@crain2008; @darling2008; @cote2016].

The uncertainty associated with complex driver interactions must therefore be taken into account when investigating environmental impacts [@cote2016], yet most research on driver effects in marine environments remains overwhelmingly focused on single driver assessments [@obrien2019]. Increasing exposure and the experiences of past ecological tragedies such as the collapse of cod fisheries [@frank2005; @dempsey2018] and the decline of the beluga and right whale populations [@plourde2014] together urge the need to characterize the distribution, intensity and \textcolor{red}{\sout{overlap}} \textcolor{blue}{co-occurrence} between drivers in the St. Lawrence System. \textcolor{blue}{The impacts of drivers, whether individually or in combination, depend on species-specific vulnerabilities. Driver co-occurrence alone is therefore not sufficient evidence to claim interactions among drivers. However, as drivers must co-occur to interact, the likelihood of interactions will increase with the number of drivers co-occurring.} \textcolor{red}{\sout{This will}} \textcolor{blue}{Describing drivers will therefore} provide critical information on areas most exposed to \textcolor{red}{\sout{cumulative}} drivers \textcolor{red}{\sout{and on the interaction potential of drivers}} in the St. Lawrence. It is also a necessary step towards the holistic and integrated management of the St. Lawrence System.

\textcolor{blue}{Holistic approaches typically involve, but are not limited to, the following steps: selecting and describing valued ecosystem components (\textit{e.g.} habitats and species) and drivers (\textit{e.g.} marine traffic and ocean acidification), assessing the exposure and vulnerability of valued components to drivers, selecting a proper spatio-temporal scale, monitoring, and public and stakeholder participation} [@dube2001].
Gathering environmental
\textcolor{red}{\sout{data for large scale, systematic}}
\textcolor{blue}{knowledge for holistic}
initiatives can, however, be a very challenging and time consuming – not to say painful – process. On one hand, there is an overwhelming and expanding wealth of data available. Such information overload may inhibit our ability to make decisions based on scientific information, promote massive effort duplication, disproportionately appropriate research funds to certain sectors, and obscure knowledge gaps amid a sea of information [@eppler2004]. On the other hand, crucial data are lacking and remain largely unavailable or inaccessible for a variety of reasons, including proprietary rights, lack of organizational time, capacity and training, and in some \textcolor{red}{\sout{rare}} cases unwillingness to share, curtailing our ability for appropriate decision-making.

There are now initiatives that \textcolor{blue}{facilitate the data gathering process} \textcolor{red}{\sout{address this issue}} by assembling, organizing and sharing environmental knowledge, such as the Ocean Biogeographic Information
...
effects of multiple drivers [@dafforn2016; @stock2018].
~~An additional objective
thus emerged in the process of addressing our initial goals:
sharing information about the
distribution and intensity of drivers of change in the St.Lawrence.~~

Our main ~~objective~~ \textcolor{blue}{goal} in this study is to characterize the distribution and
intensity of drivers in the St. Lawrence. More specifically, ~~we aim~~ \textcolor{blue}{our objectives are} to: 1)
identify areas of high cumulative exposure\textcolor{red}{\sout{, 2) identify drivers that are likely to interact in the St. Lawrence}} and \textcolor{red}{\sout{3)}} \textcolor{blue}{2)} characterize areas with similar cumulative exposure regimes.
\textcolor{red}{\sout{Here we focus solely on drivers, one of four elements required to evaluate environmental impacts, *i.e.* drivers, ecosystem components of interest (*e.g.* habitats or species) and the exposure and the vulnerability of the ecosystem components to drivers.}}
\textcolor{blue}{Here, we focus solely on drivers, aiming at better understanding their distribution in the St. Lawrence and at facilitating the widespread availability of their characterization for holistic assessments and management approaches.}
\textcolor{blue}{An additional objective emerged while addressing the main objective of this
manuscript: sharing information about the distribution and intensity of drivers of change in the St.Lawrence.}
We achieve ~~these objectives~~ \textcolor{blue}{this} with the development of an
...
evolve ~~with the addition of information and threats to~~
\textcolor{blue}{as knowledge accumulates and new threats emerge in}
the St-Lawrence ecosystems.
\textcolor{blue}{Here, we present its guidng principles and accompanying tools.}
...
The St. Lawrence System is formed by the \textcolor{blue}{St. Lawrence} Estuary and the Gulf of St. Lawrence
...
connexion to the Great Lakes Basin\textcolor{red}{\sout{. The Basin}}\textcolor{blue}{, which }is home to over 45 million
North-Americans, *i.e.* 15 and 30 million in Canada and the United States,
respectively. The St. Lawrence itself boasts a much lower
...
\textcolor{red}{\sout{The list of drivers for which we sought data was informed by}} \textcolor{blue}{Drivers selection was informed by} a global
cumulative impact assessment initiative [@halpern2008a; @halpern2015a; @halpern2019] \textcolor{blue}{and available from the National Center for Ecological Analysis and Synthesis (NCEAS) online data repository (Halpern et al. 2015)},
...
\textcolor{red}{\sout{We integrated datasets from regional experts and also use global data from the global cumulative impact assessment initiative [@halpern2008a; @halpern2015a]  available from the National Center for Ecological Analysis and Synthesis (NCEAS) online data repository [Table 1; @halpern2015].}}
...
\textcolor{blue}{Where regional data on drivers were unavailable, available global data at a resolution adequate for the scale of the St. Lawrence System were used instead (*e.g.* marine pollution).}
...
\textcolor{blue}{As in Halpern et al. 2019, drivers with non-normal frequency distributions were log-transformed
to avoid underestimating intermediate driver values. All drivers were also scaled
between 0 and 1 to allow driver comparisons. The 99th quantile of individual
driver distribution was used as the upper bound for scaling to control for
extreme values that may or may not be real observations.}
...
1 $km^2$ hexagonal cells in which all drivers were integrated (Figure S2)}

## \textcolor{red}{\sout{Driver interactions}} \textcolor{blue}{Driver co-occurrence}

\textcolor{red}{\sout{Interactions among drivers were investigated using the cumulative footprint ($F$) between pairs of drivers (objective 2). The intensity at which pairs of drivers co-occur was evaluated using a two-dimensional kernel density estimate. As there are 231 pairwise combinations between 22 drivers, we focus on a single example using hypoxia and demersal destructive fisheries, two drivers known to occur mainly in deeper areas of the St. Lawrence and, hence, an interaction between the effects of the two drivers could be anticipated.}}
\textcolor{blue}{As an introduction to indicators of cumulative exposure (objective 1), we begin by presenting a simplifed 2-driver example focusing on hypoxia and demersal destructive fisheries, two drivers known to occur mainly in deeper waters of the St. Lawrence. Driver co-ccurrence was evaluated spatially by summing the scaled intensity of drivers in each grid cell. The intensity at which pairs of drivers co-occur was evaluated using a two-dimensional kernel density.}
...
In order to \textcolor{red}{\sout{better capture potential interactions between drivers (objective 2) and to}}identify areas with similar exposure regimes (objective \textcolor{red}{\sout{3}}\textcolor{blue}{2}), we identify threat complexes using a clustering approach [*e.g.* see @bowler2019]. We use the term clusters in presenting the methods, but use threat complex when discussing the results on cumulative exposure regimes.
...
Hypoxia is mainly distributed in the Laurentian, Anticosti and Esquiman Channels, with the head of the Channels most exposed (Figure \ref{kernel}A). Demersal destructive fisheries are located along the Laurentian Channel, the heads of the Anticosti and Esquiman Channels and around the Magdalen Islands (Figure \ref{kernel}B). By combining both drivers, we can observe that hypoxia and demersal destructive fisheries co-occur mostly at high relative intensity (Figure \ref{kernel}D) in the vicinity of the Anticosti Gyre and the heads of the Esquiman and Anticosti Channels (Figure \ref{kernel}C~~, Box 1~~).

Fisheries in the St. Lawrence have historically affected biodiversity distribution and habitat quality [@moritz2015]. Concurrently, hypoxia decreases overall habitat quality, but triggers species-dependent responses ranging from adaptation [*e.g.* northern shrimp *Pandalus borealis* and Greenland halibut *Reinhardtius hippoglossoides*; @pillet2016] to reduced growth rates [@dupont-prinet2013] and avoidance of oxygen-depleted habitats [*e.g.* Atlantic cod *Gadus morhua*; @chabot2008] to increased mortality [*e.g.* sessile benthic invertebrates; @eby2005; @gilbert2007; @belley2010]. Certain species may thus be adversely affected by fisheries and withstand hypoxia but still experience a decrease in prey availability, while others may be deleteriously affected by the compounded effect of both drivers [@deleo2017].
...
While we cannot ascertain that high exposure areas are the most impacted, we can safely predict that these are the areas where studying ecosystem state will be the most complex due to the uncertainty associated with driver \textcolor{red}{\sout{interactions}} \textcolor{blue}{co-occurrence}, an uncertainty bound to increase rapidly with the number of \textcolor{red}{\sout{interacting}} \textcolor{blue}{co-occurring} drivers [@cote2016].
...
\textcolor{red}{\sout{Of particular concern are threat complexes 3 and 6, which are the two most exposed complexes in the St. Lawrence and are characterized by distinct cumulative exposure regimes Between them, they capture most of the coastal and offshore hotspots identified in the St. Lawrence.}}
\textcolor{blue}{Threat complexes 3 and 6 capture most coastal and offshore hotspots and are the two most exposed complexes in the St. Lawrence.}
They \textcolor{red}{\sout{also}} offer some insight into the potential importance of considering
...
management and open science. We identified four guiding principles \textcolor{blue}{(Sections 5.1 to 5.4)} to meet this
...
Through *eDrivers*, these observations will be iteratively improved towards an increasingly robust assessment of cumulative exposure and threat complexes as gaps in knowledge are addressed or approaches to describe drivers are refined. Arguably, the most meaningful benefit anticipated from *eDrivers* will be the gain in efficient access to comparable data-based knowledge on the exposure of ecosystems to multiple threats. \textcolor{blue}{We provide an example of the ease with which this can be accomplished using the \textit{eDrivers} R package to reproduce figure \ref{kernel} (Box 1).} This could pay quick scientific and management dividends by \textcolor{blue}{efficiently} drawing on the knowledge and efforts of a wide range of contributors, by expanding avenues of scientific inquiry, by decreasing overall effort duplication and research costs, and by increasing research efficiency [@franzoni2014].
...
\textcolor{red}{\sout{While the focus of our paper has been on the description of drivers in the St. Lawrence, drivers are but one of the 4 elements required to properly evaluate environmental impacts. This knowledge node should be weaved with other, comparable, nodes required for integrated management approaches such as species distribution and marine protected areas. Ultimately, all of these knowledges could be weaved together through social-ecological metanetworks (Dee et al. 2017).}}
\textcolor{blue}{The next step will be the inclusion of other types of knowledge to our initiative. Our focus has been on a single element required for fully operational impact assessments. Data that provide knowledge on the exposure of ecosystems to drivers are called stressor-based indicators} [@dube2001; @dube2003; @jones2016]. \textcolor{blue}{These indicators efficently identify potential local impacts and can be proactively linked to decision-making, yet assume complete knowledge of drivers but fail to diagnose impacts on valued components or non-additive effects} [@dube2001; @dube2003]. \textcolor{blue}{In contrast, effect-based indicators are direct measurements of valued components (\textit{e.g.} species abundance and biodiversity) and inherently capture the effects of multiple drivers} [@dube2001; @dube2003]. \textcolor{blue}{Whereas effect-based indicators are considered superior to stressor-based indicators, they fail to ascribe observed effects to specific drivers} [@dube2001; @dube2003]. \textcolor{blue}{Stressor-based and effect-based indicators are, therefore, both required to diagnose causes of ecosystem change} [@dube2003; @jones2016]. \textcolor{blue}{As a collection of knowledge on stressor-based indicators, \textit{eDrivers} should be weaved with other, comparable, collections of knowledge describing valued ecosystem components that can be linked to drivers and allow for a better understanding of cumulative impacts. Ultimately, interdisciplinary colections of knowledge could be weaved together through social-ecological metanetworks analyses} [@dee2017].
...
SLGO partly addresses this issue. \textcolor{blue}{We also wish to provide users with enhanced capabilities and flexibility in using the interactive tool and R package. This could include creating automatic reports and more flexibility for user-defined driver-based indicators.}
...
\begin{figure}[H]
\centering
\includegraphics{../Figures/egsl.png}
...
\caption{Distribution of threat complexes in the Estuary and Gulf of St. Lawrence (upper panel). Threat complexes (a term coined by Bowler et al., 2019) are areas with similar cumulative driver exposure regimes. Mean intensity of all coastal (red), climate (green), fisheries (blue) and marine traffic (purple) drivers within each threat complex (lower panel). Refer \textcolor{red}{\sout{tp}} \textcolor{blue}{to} Table 1 for acronym meaning and to the Supplementary Materials for more details.}
\label{cluster}
\end{figure}
