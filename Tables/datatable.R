...
df[1,] <- c("Climate", "Aragonite", 'Lat/long', 'August-September', '2018', '$\\Omega$ $Aragonite$', '[@starr2019]','ARAG')
...
df[7,] <- c("Climate", "Sea level rise", 'Modeled 0.25 degree', '10 days', '1992-2012', '$mm$','[@halpern2015b]','SLR')
df[8,] <- c("Marine traffic", "Shipping", '0.1 degree', 'Event based', '2003-2011', '$n$ lanes', '[@halpern2015b]','SHP')
...
df[14,] <- c("Coastal", "Coastal development", '15 arc-second', 'Annual', '2015-2016', '$nanoWatts$ $cm^{-2}$ $sr^{-1}$', '[@eog2019]','CD')
df[15,] <- c("Coastal", "Direct human impact", 'Modeled 1 $km^2$', 'Annual', '2011', '$population$ $10km^{-2}$', '[@halpern2015b]','DHI')
df[16,] <- c("Climate", "Hypoxia", 'Lat/long', 'August-September', '2018', '$ml$ $L^{-1}$', '[@starr2019]','HYP')
df[17,] <- c("Marine traffic", "Invasive species", 'Modeled 1 $km^2$', 'Annual', '2011', '$t$ port volume', '[@halpern2015b]','INV')
df[18,] <- c("Coastal", "Inorganic pollution", 'Modeled 1 $km^2$', 'Annual', '2000-2001', '-', '[@halpern2015b]','IP')
df[19,] <- c("Marine traffic", "Marine pollution", 'Modeled 1 $km^2$', 'Event based & annual', '2003-2011 & 2011', '$n$ lanes + $t$ port volume', '[@halpern2015b]','MP')
df[20,] <- c("Coastal", "Nutrient import", 'Modeled 1 $km^2$', 'Annual', '2007-2010', '$t$ fertilizer', '[@halpern2015b]','NI')
df[21,] <- c("Coastal", "Organic pollution", 'Modeled 1 $km^2$', 'Annual', '2007-2010', '$t$ pesticide', '[@halpern2015b]','OP')
